...
import type { Building, CombatEvent, Drone, DroneId, HudState, InputSnapshot, NetworkAction, Projectile } from './types';
...
export class GameWorld {
  ...
  private updatePlayer(dt: number, input: InputSnapshot) {
    ...
    const maxSpeed = boosting ? BALANCE.player.boostSpeed : BALANCE.player.maxSpeed;
    const accel = boosting ? BALANCE.player.accel * 2.05 : BALANCE.player.accel;

    if (boosting) {
      player.boost = clamp(player.boost - dt * BALANCE.player.boostDrain, 0, 1);
    } else {
      player.boost = clamp(player.boost + dt * BALANCE.player.boostRecharge, 0, 1);
    }
  ...
  private updateEnemies(dt: number) {
    ...
    for (const enemy of this.drones) {
      ...
      const desiredRange = BALANCE.enemy.desiredRangeMin + enemy.aiPhase * BALANCE.enemy.desiredRangeSpread;
      ...
      enemy.velocity = add(enemy.velocity, scale(desired, BALANCE.player.accel * (BALANCE.enemy.accelScale + threat * 0.12) * dt));
      ...
      if (range < 125 && aimAlignment > 0.84 && enemy.laserCooldown <= 0 && player.respawnTimer <= 0) {
        enemy.laserCooldown = Math.max(BALANCE.enemy.minFireCooldown, BALANCE.enemy.baseFireCooldown - threat * 0.24) + Math.random() * 0.28;
  ...
  private damageDrone(drone: Drone, amount: number, ownerId: DroneId) {
    ...
    drone.respawnTimer = drone.team === 'player' ? 2.2 : BALANCE.enemy.respawnDelay;
    drone.velocity = vec3();
    if (drone.team === 'player') this.playerDeaths += 1;
    this.emitEvent(
      'explosion',
      drone.position,
      drone.team === 'player' ? 0x52f7ff : 0xff3f8f,
      3.2,
      drone.team === 'enemy' ? `DESTROYED ${drone.name}` : 'YOU WERE DESTROYED',
      drone.team === 'enemy' ? 100 : 0,
    );
    ...
    for (const drone of this.drones) {
      ...
      for (const building of this.buildings) {
        ...
        if (drone.hp <= 0) {
          drone.respawnTimer = drone.team === 'player' ? 2.2 : BALANCE.enemy.respawnDelay;
          if (drone.team === 'player') this.playerDeaths += 1;
          this.emitEvent('explosion', drone.position, drone.team === 'player' ? 0x52f7ff : 0xff3f8f, 3, drone.team === 'player' ? 'COLLISION FATAL' : `DESTROYED ${drone.name}`, drone.team === 'enemy' ? 100 : 0);
          this.checkRoundEnd();
        }
      }
    }
...
function createDrone(id: DroneId, name: string, team: Drone['team'], position: Vec3): Drone {
  return {
    id,
    name,
    team,
